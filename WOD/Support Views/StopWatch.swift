...
class StopWatch: ObservableObject {
    ...
    private var currentRecords = [RecordItem]() {
        didSet {
            self.records = currentRecords.reversed()
        }
    }
    ...
    func record() {
        if let firstRecord = self.records.first {
            let difference = self.counter - firstRecord.count
            self.currentRecords.append(RecordItem(count: self.counter, diff: difference))
        } else {
            self.currentRecords.append(RecordItem(count: self.counter))
            self.paused = !self.paused
            self.sourceTimer?.suspend()
    ...
    func reset() {
        ...
        self.counter = 0
        self.currentRecords = [RecordItem]()
    }
